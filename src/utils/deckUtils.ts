...
export const deckUtils = {
  getDueCards: (cards: Card[]): Card[] => {
    const now = Date.now();
    return cards.filter(card => (card.due || card.nextReviewDate || 0) <= now);
  },
  ...
  getSmartDecks: (cards: Card[], decks: Deck[]) => {
    ...
    return decks.map(deck => {
      ...
      const dueCount = deckCards.filter(card => (card.due || card.nextReviewDate || 0) <= now).length;
      return {
        ...deck,
  ...
  getDeckMastery: (cards: Card[]): number => {
    ...
    const totalScore = cards.reduce((acc, card) => {
      // Use FSRS stability if available, otherwise fallback to interval
      if (card.stability !== undefined) {
        if (card.stability >= 21) return acc + 100;
        if (card.stability >= 7) return acc + 80;
        if (card.stability >= 3) return acc + 50;
        if (card.stability > 0) return acc + 20;
      } else {
        const interval = card.interval || 0;
        if (interval >= 21) return acc + 100;
        if (interval >= 7) return acc + 80;
        if (interval >= 3) return acc + 50;
        if (interval > 0) return acc + 20;
      }
      return acc;
    }, 0);
  ...
  getSmartStartDeck: (decks: Deck[], cards: Card[]): string | null => {
    ...
    const deckStats = decks.map(deck => {
      ...
      const dueCount = deckCards.filter(c => (c.due || c.nextReviewDate || 0) <= Date.now()).length;
