...
export class DatabaseService {
  ...
  async getSettings(): Promise<UserSettings> {
    ...
    return (record as { key: string; value: UserSettings } | undefined)?.value || { targetRetention: 0.90 };
  }

  async saveSettings(settings: UserSettings) {
    const db = await this.dbPromise;
    return db.put('settings', { key: 'user', value: settings } as { key: string; value: UserSettings });
  }
}
